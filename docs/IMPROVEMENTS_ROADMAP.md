...
[~] P2-10 Wire cross-language vector test into CI as required gate         (Rust↔TS `tests/vectors/` + Phase 3.4 Rust↔circuit `gen_circuit_vectors` + `circuits/npm test` in `cross_language_vectors`; SOLID-SEC-010 still 3/10 primitives -- expand coverage)
