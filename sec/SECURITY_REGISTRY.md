...
| HIGH      | 4    | 0           | 19    | 0        | 0         | 23    |
| MEDIUM    | 10   | 0           | 12    | 0        | 0         | 22    |
| LOW       | 4    | 0           | 6     | 0        | 0         | 10    |
| INFO      | 4    | 0           | 2     | 0        | 0         | 6     |
| **Total** | 22   | 0           | 45    | 0        | 0         | 67    |
...
| SOLID-SEC-048   | HIGH     | Fixed  | `register_issuer` BJJ prime-order subgroup check exceeds 1.4M CU per-tx ceiling on BPF.  Closed 2026-05-02 via Phase E (Option B): small dedicated Groth16 circuit `circuits/bjj_subgroup_proof.circom` proves `[r] * P == identity AND on_curve(P) AND P != identity` off-chain (~50-150ms per registration); on-chain `solid_light::groth16::verify_groth16_proof::<2>` consumes a 256-byte proof inside `register_issuer` (~285K CU per Groth16 verify; total `register_issuer` ~365-400K CU under the 1.4M ceiling).  `sec007-skip-onchain` Cargo feature DELETED.  `Sec007Bypass` event DELETED.  Subgroup VK uploaded via new `init_subgroup_verifier` / `store_subgroup_vk_chunk` / `finalize_subgroup_vk` ix family with the same SOLID-SEC-006 48h-timelock rotation pattern as the batch VK.  Off-chain SDK predicate `isInPrimeOrderSubgroup` retained as UX pre-submit gate (no longer load-bearing).  E2E green end-to-end against the no-bypass build: `verified: true` + replay-rejection.  Phase E commits: `cbbe088` (E.1 lift Groth16 primitives to solid-light), `87cdd34` (E.2 SubgroupVerifierConfig PDA), `0a97099` (E.3 register_issuer wiring + bypass dropped + host round-trip test), `30343b5` (E.4 SDK + scripts).  Sibling: SOLID-SEC-083 (auth-race hardening) + the snarkjs-workers + IDL drift discipline notes. |
...
| SOLID-SEC-054   | MEDIUM   | Fixed  | `verify_batch_proof` ix data 1324 bytes exceeded Solana's 1232-byte legacy-tx packet limit (B13).  Closed 2026-05-01 via **Option 2 (buffer-account / chunked upload)**: three new ixs in `programs/zk-verifier/src/lib.rs` -- `init_proof_buffer` (allocates a per-payer scratch PDA), `upload_proof_chunk` (idempotent chunked write), `verify_batch_proof_v2` (reads staged proof + does B13 on-chain reconstruction + Groth16 verify + atomic nullifier PDA init + closes buffer for rent refund).  SDK orchestration in `ts-sdk/packages/verifier/src/index.ts::verifyOnChainV2`.  Each tx fits comfortably under the 1232-byte cap regardless of how many wire inputs the circuit grows to.  E2E green: `tVYvkyTt55r8RCf3LhVMTmBrKzr5HFtDJcwKM5tFHXerUaxmQSMsZQoKLR8HXbDMu2gYBjNwxC9gaSpdA1oMmCX` returns `verified: true` + replay-rejection on the next attempt.  Regression gates: 6 ProofBuffer slot-partition / payload-size invariant tests in zk-verifier, plus the host-side Groth16 round-trip (`groth16_host_verify_round_trip`).  Three latent bugs (LB1 `Vec<[u8; 32]>` deser, LB2 SEC-005 timestamp BE/LE, LB3 reconstructed Merkle slot byte-reverse) shipped in the original 2026-04-29 arc and remain in tree.  Two further latent bugs surfaced and were fixed on 2026-04-30: LB4 (binding hash-family was Keccak; circuit needs Poseidon -- folded into SEC-059 atomic recompute) and LB5 (G2 byte-order swap; tracked as SEC-067). |
| SOLID-SEC-058   | CRITICAL | Fixed  | Off-chain proving artifacts (.wasm, .zkey) fetched from CDN with no SHA-256 gate (sibling to SEC-041).  Closed 2026-04-30: new `ts-sdk/packages/sdk/src/artifact_integrity.ts` module with `loadAndVerifyArtifact` that hashes downloaded bytes and refuses on mismatch.  Three pin sources (env var > sidecar `.sha256` > config constant); CDN-loaded artifacts with no pin throw.  Sidecars at `circuits/build/{batch_credential_query.wasm,zkey,verification_key}.sha256`.  Regression gate: 14 unit tests in `tests/unit/artifact_integrity.test.ts`. |
| SOLID-SEC-059   | CRITICAL | Fixed  | `update_issuer_tree_root` was a single-key root-injection primitive (caller-supplied 32-byte value written verbatim into binding).  Closed 2026-04-30: ix accepts `(new_root, new_leaf, leaf_index, poseidon_proof_path: Vec<u8>)` and runs on-chain Poseidon-Merkle recompute via `solid_light::cpi_helpers::compute_poseidon_merkle_root`; refuses any push whose recompute does not equal `new_root`.  Atomic-update variant integrated into `append_issuer_leaf`, `revoke_issuer_atomic`, `request_withdrawal_atomic` so the binding is updated in the same ix as the leaf change.  Sibling closures: `schema_registry::update_tree_root` and `update_global_root` got the same Poseidon-recompute integrity gate (depth=20, 800K cuIx).  Together these close the H1 + CRIT-2 + LB4 architectural triple.  Regression gates: e2e walks the full append+update flow; CU baselines pinned in `docs/CU_BUDGET.md`. |
| SOLID-SEC-061   | HIGH     | Fixed  | Stuck-stake post-Cooldown trap for tree-enrolled issuers (SEC-044's amendment routed full withdrawals through `revoke_issuer_atomic` but added no compensating stake-recovery path).  Closed 2026-04-30: new `withdraw_after_revoke` ix accepting Revoked issuers with `staked_amount > 0` and 24h DAO dispute window via `cooldown_ends_at` reuse.  New `StakeWithdrawn` event.  Regression gate: handler + accounts struct + new error codes (`IssuerNotRevoked`, `DisputeWindowOpen`). |
| SOLID-SEC-062   | HIGH     | Fixed  | `issue_credential` accepted non-canonical (BN254 `c >= p`) commitments, and `register_issuer` accepted non-canonical BJJ x/y.  Closed 2026-04-30: new `solid_core::poseidon::is_canonical_bn254_le` (~100 CU on BPF, depth-32 byte compare); `require!` gates on `commitment` in `issue_credential` and on `bjj_pub_key_x/y` in `register_issuer`.  Regression gate: 10 host tests in solid-core covering modulus edges + padding-shift. |
| SOLID-SEC-063   | HIGH     | Fixed  | Schema-hash preimage bound only `(name, version, num_fields)` -- two registrations differing only in field semantics or category collided.  Closed 2026-04-30: preimage widened to 5-input Poseidon over (name_h, version_e, count_e, fnames_h, cat_h) where each `*_h` is a `poseidon_compress_bytes` Merkle-Damgard absorb (BPF-clean via `sol_poseidon`).  TS mirror in `@solid-protocol/core::computeSchemaHash`.  Regression gate: 4 host tests pinning determinism, sensitivity, padding-shift collision rejection. |
...
| SOLID-SEC-080   | MEDIUM   | Fixed  | NF-03 (audit 2026-05-01).  Closed 2026-05-01: lifted the `verify_*_binding_anchor` helper from issuer-registry into a generic `solid_light::cpi_helpers::verify_binding_root_anchor(expected_root, old_leaf, leaf_index, path)` and wired it into BOTH `update_tree_root` (binding root at bytes 72..104) and `update_global_root` (binding root at bytes 8..40).  Both ixs now take a new `old_leaf: [u8; 32]` parameter; the anchor recomputes Poseidon-Merkle root from `(old_leaf, leaf_index, path)` and asserts equality with `binding.current_root` BEFORE the existing SEC-059 self-consistency check fires.  Mismatched paths fail with `ErrorCode::BindingRootStale` -- consumers must re-fetch the live binding state and re-derive the path.  Fresh-binding sentinel: when `current_root == [0u8; 32]` (the post-`initialize_*_binding` placeholder), the anchor short-circuits Ok() -- the empty-tree Poseidon root is non-zero so it would otherwise reject every first update.  Real CU cost: ~0 on first update (sentinel path), ~50-80K on subsequent updates (depth-20 Poseidon recompute).  prove.ts now passes `[0u8; 32]` as `old_leaf` (it always issues the first credential per fresh e2e run).  Regression gates: 5 host tests in `crates/solid-light/src/cpi_helpers.rs::tests` cover sentinel short-circuit / matching recompute / wrong_old_leaf / wrong_leaf_index / stale_path_against_advanced_binding cases.  E2E green end-to-end on a clean validator (verified the sentinel path; the full-anchor path is exercised by the unit tests).  CU baselines refreshed; SEC-046 gate clean. |
| SOLID-SEC-081   | MEDIUM   | Fixed  | NF-05 (audit 2026-05-01): `scripts/check_program_ids.py` validated Anchor.toml + `programs/*/src/lib.rs::declare_id!` + `cpi_helpers.rs` constants + `deployments/*.json` but did NOT parse TS files for hand-rolled program-id literals.  Closed 2026-05-01: extended the gate with `scan_ts_files_for_pubkey_literals` + `TS_PROGRAM_ID_OWNERS` allowlist (4 owner files: `ts-sdk/packages/{core,sdk,light}/src/...` + `tests/integration/01_registry_init.test.ts`).  Walks every other .ts file under `ts-sdk/packages/*/src/`, `scripts/`, `tests/integration/` and rejects any non-allowlisted file that hard-codes a canonical program ID -- consumers must `import { PROGRAM_PUBKEYS } from '@solid-protocol/core'` instead.  Owner files MUST carry the literal canonical IDs (typo or deletion both trip the gate).  Regression suite at `scripts/test_check_program_ids.py` (5 tests: clean fixture / Rust declare_id drift / TS owner typo / unauthorized TS file / owner missing ID); wired into CI as a second step on the `program_id_consistency` job.  ROOT now overridable via `SOLID_REPO_ROOT` env var so tests can build synthetic fixtures without touching the live tree. |
...
| SOLID-SEC-083   | HIGH     | Fixed  | SEC-048 Phase E hardening (discovered + closed 2026-05-02 during the same e2e run that closed SEC-048): `init_subgroup_verifier` was a first-caller-wins singleton with no constraint tying its authority to `registry_config.authority`.  On a fresh public cluster, an attacker watching for `anchor deploy` could front-run the operator's `scripts/initialize.ts` step [8/8], become the `SubgroupVerifierConfig.authority`, then upload a permissive subgroup VK that admits any pubkey -- nullifying the SEC-048 Phase E.3 soundness gate without leaving the operator any in-protocol recovery (would require a program redeploy).  Closed 2026-05-02: added `registry_config: Account<'info, RegistryConfig>` to `InitSubgroupVerifier` + the constraint `authority.key() == registry_config.authority @ ErrorCode::Unauthorized`.  This forces the SubgroupVerifierConfig.authority to be the SAME key that already controls the registry, so the only race window remaining is the (already-existing, equally narrow) one for `initialize_registry`.  Operator runbook unchanged because the existing script flow (initialize_registry then init_subgroup_verifier as steps 1 and 8 of the same script) already satisfies the constraint.  Regression gate: source-level `sec_083_unauthorized_error_code_pinned` test (the bankrun integration test that exercises the constraint with a hostile authority is tracked at `tests/integration/12_subgroup_vk_init_authority_race.test.ts` (TODO; folds into the BACKLOG #7 02-11 suite)). |
