...
| SOLID-SEC-054   | MEDIUM   | Open (interim partial) | `verify_batch_proof` ix data 1324 bytes exceeded Solana's 1232-byte legacy-tx packet limit (B13).  Originally-recommended path was (1) on-chain reconstruction + (3) Versioned-tx + ALT.  As of 2026-04-29 (1) and (3) shipped in code -- the handler reconstructs 11 of 32 slots (globalRoot, merkleRoots[0..3], schemaHashes[0..3], issuerTreeRoot, verifierAddress) from the accounts already on the ix surface; SDK uses VersionedTransaction + AddressLookupTable.  But (1)+(3) is **architecturally 37 bytes over** the 1232-byte cap once a `setComputeUnitLimit` ix is prepended (verify_batch_proof needs ~285K-345K CU vs 200K default).  Pivot to **Option 2 (buffer-account / chunked upload)** in next session per `docs/REMEDIATION_OPTIONS_ARCHIVE.md` §1.1 + `docs/E2E_BLOCKERS.md` B13.  This entry stays Open until the buffer-account flow lands and `npm run e2e` returns `verified: true`.  Three latent bugs LB1 (`Vec<[u8; 32]>` BorshDeserialize fails on BPF -> changed handler arg to `Vec<u8>`), LB2 (SEC-005 timestamp slot was read LE but SDK encodes BE), and LB3 (reconstructed Merkle slots have BE/LE asymmetry vs holder's `bufToDecimal` LE-decode -> byte-reverse on copy) shipped in the same arc; should be split into atomic commits + own SEC-XXX entries (siblings of SEC-031) per L9 of `plan/SESSION_LOG_2026-04-29.md`.  Regression gates already in tree: 6 cargo unit tests (slot-partition invariants in zk-verifier), 13 cargo unit tests for the new `extract_active_*` helpers in solid-light (round-trip equivalence with existing `verify_*` paths). |
